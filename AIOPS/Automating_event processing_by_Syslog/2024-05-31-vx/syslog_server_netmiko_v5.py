...
def handle_event(event_type, message, dev_ip_addr, interface=None):
    ...
    # 로그 데이터를 텍스트 파일로 저장
    save_log_to_text_file(file_base_name, event_type, message)

# 텍스트 파일 저장 함수
def save_log_to_text_file(file_base_name, event_type, message):
    try:
...
def save_logs_to_csv():
    ...
    while True:
        if log_data:  # log_data 리스트에 로그가 있는 경우
            # 리스트 컴프리헨션으로 log_data를 log로 변경 후 preprocess_log()함수로 전처린 후 valid_logs로 저장
