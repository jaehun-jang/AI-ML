...
# 임계값 설정
TX_POWER_THRESHOLD = -5.0
RX_POWER_THRESHOLD = -5.0
...
def read_cli_and_write_to_csv(m_box):
    ...
    while True:
        ...
        print("[DEBUG] collected_data: ", collected_data)
        # 임계값 체크 및 이벤트 트리거
...
def train_model(data):
    ...
    if len(data) < 10:  # 최소 데이터 수집 개수 설정
        return
...
def predict_link_down(results):
    ...
    prediction = model.predict_proba(features)[0, 1]  # 링크 다운 확률
...
def collect_and_train():
    global collected_data
    while True:
        print("[DEBUG] collected_data TRUE")
        if collected_data:
            train_model(collected_data)
            collected_data = []
